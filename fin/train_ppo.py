...
def main(args, env_name, number, seed):
    ...
    env.reset(seed=seed)  # ✅ 用 reset(seed=...) 来设置种子
    env.action_space.seed(seed)

    env_evaluate = gym.make(env_name)
    env_evaluate.reset(seed=seed + 100)  # ✅ 避免评估和训练用相同的轨迹
    env_evaluate.action_space.seed(seed + 100)  # When evaluating the policy, we need to rebuild an environment
...
if __name__ == '__main__':
    ...
    env_index = 1
    main(args, env_name="LunarLander-v2", number=1, seed=0)
